...
const PromotionsTab = ({ data, onRefresh, showMessage }) => {
  ...
  return (
    <div>
      <div className={styles.add_button_wrapper}>
        <button onClick={() => openPromotionModal()} className={styles.add_btn}>
          <FiPlus /> Добавить акцию
        </button>
      </div>
      ...
      <Modal isOpen={promotionModal.open} onClose={() => setPromotionModal({ open: false, editing: null })} title={promotionModal.editing ? 'Редактировать акцию' : 'Добавить акцию'} size="medium">
        <form onSubmit={handlePromotionSubmit} className={styles.modal_form}>
          ...
          <div className={styles.modal_actions}>
            ...
            <button type="button" onClick={() => setPromotionModal({ open: false, editing: null })} className={styles.cancel_btn}>
              <FiX /> Отмена
            </button>
          </div>
        </form>
