...
import { FiPlus, FiEdit2, FiLock, FiUnlock, FiSave, FiX, FiEye, FiEyeOff } from 'react-icons/fi';
import styles from '../AdminPanel.module.css';

const UsersTab = ({ data, onRefresh, showMessage }) => {
  ...
  const handleUserSubmit = async (e) => {
    ...
    try {
      ...
      onRefresh();
      showMessage(userModal.editing ? 'Пользователь обновлен' : 'Пользователь создан');
  ...
  return (
    <div>
      <div className={styles.add_button_wrapper}>
        <button onClick={() => openUserModal()} className={styles.add_btn}>
      ...
      <Modal isOpen={userModal.open} onClose={() => setUserModal({ open: false, editing: null })} title={userModal.editing ? 'Редактировать пользователя' : 'Добавить пользователя'} size="medium">
        <form onSubmit={handleUserSubmit} className={styles.modal_form}>
          ...
          <div className={styles.modal_actions}>
            ...
            <button type="button" onClick={() => setUserModal({ open: false, editing: null })} className={styles.cancel_btn}>
              <FiX /> Отмена
            </button>
          </div>
        </form>
